...
{
    protected $signature = 'school:import-classes {path : File Excel danh sách lớp học}
        {--school-year=2025-2026 : Năm học}
    ...
    protected $description = 'Import lớp học theo Mã lớp, fallback Tên lớp + năm học.';

    public function handle(SchoolClassImportService $service): int
    {
        ...
        $analysis = $service->analyzePath($path, basename($path), $schoolYear, $headerRow);
        ...
        if ((bool) $this->option('commit') && ! (bool) $this->option('dry-run')) {
            ...
            $count = $service->commit($service->previewPath($path, basename($path), $schoolYear, $headerRow));
            $this->info("Đã import {$count} lớp học.");
        } else {
